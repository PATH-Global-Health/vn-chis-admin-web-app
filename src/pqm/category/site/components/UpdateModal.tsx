...
import { Site, SiteUM } from '@pqm/category/site/site.model';
...
const CreateModal: React.FC<Props> = ({ data, onClose, onRefresh }) => {
  ...
  const onSubmit = useCallback(
    async (d: SiteUM) => {
      await fetch(siteService.updateSite(d));
      onRefresh();
      onClose();
      reset({});
    },
  ...
  useEffect(() => {
    register('id');
    register('order', {
      required: 'Bắt buộc nhập thứ tự',
      min: 'Thứ tự phải lớn hơn 0',
    });
        ...
        <Form>
          <Form.Group widths="equal">
            ...
            <Form.Field
              control={Input}
              label="Thứ tự"
              error={errors?.order?.message ?? false}
              value={watch('order') || 0}
