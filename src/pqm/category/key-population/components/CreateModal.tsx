...
import { KeyPopulationCM } from '@pqm/category/key-population/key-population.model';
...
const CreateModal: React.FC<Props> = ({ open, onClose, onRefresh }) => {
  ...
  const {
    ...
    setValue,
    handleSubmit,
  } = useForm<KeyPopulationCM>({
    defaultValues: {},
  });

  const disabled = !!errors?.name?.message;

  const onSubmit = async (d: KeyPopulationCM): Promise<void> => {
    const p: KeyPopulationCM = {
      name: d.name,
    };
    ...
    await fetch(keyPopulationService.createKeyPopulation(p));
    onRefresh();
    onClose();
    reset({});
  };
