...
const HospitalAccountsPage: React.FC = () => {
  ...
  const { getHospitalsLoading, dataHospital } = useSelector(
    (state) => state.admin.account.hospital
  );

  const listHospital = dataHospital!.data!;
  // .sort((a, b) =>
  ...
  const getData = useCallback(() => {
    dispatch(getHospitals({ pageIndex: currentPageIndex, pageSize }));
    dispatch(getUnitTypes());
  }, [dispatch, currentPageIndex, pageSize]);
  ...
  return (
    <>
      <DataTable
        ...
        columns={columns}
        data={listHospital}
        pageCount={dataHospital.totalPage}
        onPaginationChange={(page) => {
          setCurrentPageIndex(page.pageIndex + 1);
          setPageSize(page.pageSize);
        }}
        onSearch={(searchValue: string) => {
          console.log(
            listHospital.filter((h) =>
              deburr(h.name).includes(deburr(searchValue))
            )
          );
        }}
        tableActions={[
        ...
        rowActions={[
          ...
          },
          {
            ...
            color: "red",
            onClick: (r): void => {
              confirm(t("Confirm delete ?"), async (): Promise<void> => {
